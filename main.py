...
parser.add_option('-t', '--toulbar', action='store_true',
                  dest='toulbar', help='use toulbar2 solver')
...
parser.add_option('-T', '--time-limit', action='store', dest='time',
                  help='time limit (second)')
...
if not options.solverpath:
    parser.error('Miss path to the solver')
# print to a file
if options.sol:
...
if options.toulbar:
    params = []
    if options.time:
        params.append("-timer=" + options.time)
else:
    params = []
...
solutions = instance.get_topk(options.K, options.numrand, options.output,
                              ...
                              K2=options.numcons, istoulbar=options.toulbar)
# print solutions
